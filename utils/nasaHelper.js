...
export function validateDateString(dateString = '') {
  ...
  return date.isValid() && date.isBetween(EARLIEST_START_DATE, moment(), 'day', '[]')
}

export function getNextDateRange(oldStartDate) {
  const endDateMoment = oldStartDate ? moment(oldStartDate).subtract(1, 'days') : moment()
  const daysCount = Math.min(endDateMoment.diff(earliestStartDateMoment, 'days'), DEFAULT_DAYS_COUNT)
  const endDate = endDateMoment.format('YYYY-MM-DD')
  const startDate = moment(endDate).subtract(daysCount, 'days').format('YYYY-MM-DD')
